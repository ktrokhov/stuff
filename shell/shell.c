...
/*msg - message from function: 1-EOF,-1-odd quotes,-2-wrong &
*eol - info about end of line */						
{
...
int cd_func (char ** first_cmd)
{
	...
	if ((check_cd(first_cmd))==1) {
                        if ((first_cmd[1])==NULL) {
                                printf("Error: no options for cd)\n");
				return -1;
			}
                        else {
...
int main()
{	char ** cmd, ** dir;
	int msg=0, eol=0, qts, backmode, error=0, no_pipe=0;
	struct node *list;
	do  {	
		kill_zombies();
		backmode=0;
		dir=malloc(3*sizeof(dir[0]));
		printf("Input command>\n");
	     	list=NULL;
		while ((msg==0) && (eol==0)) 
			list=add_node(list, &msg, &eol, &backmode, &qts);
		cmd=make_command(list,dir,&error,&no_pipe);			
		if (error==-1) {
			error=0;
			free_mem(list,cmd,dir,&eol);		
			continue;
		}
		if ((list==NULL) && (msg==1)) 
			break;
		if ((list==NULL) && (eol==1) && (msg==0)) {	
			free_mem(list,cmd,dir,&eol);
			eol=0;
			continue;	
		}			
		...
		if ((cd_func(cmd))==1) { 
			if (no_pipe)	
				run_pipe(&no_pipe, &backmode, cmd, dir);
			else
				exec_cmd(cmd,&backmode,dir);
		}
/*			print_list(list);                //Control
			print_command(cmd);		 //Control			
			print_dir(dir);		 	//Control
*/
		free_mem(list,cmd,dir,&eol);
	} while (!msg);
	kill_zombies();
